...
<html>
    ...
    <body>
                ...
                <table id="TabelasConversa"> 
                    ...
                    <%                                            
                        try {
                            ...
                            String voce = null;
                            cDB.conectar();
                            lista = cDB.listarConversa(usuarioId, 1);
                            cDB.desconectar();
                    ...
                    <table>
                        <td align='right'>
                            <form  action="enviar_mensagem.do?idDestinatario=0" method="post" enctype="multipart/form-data" >
                                <input style="display: none" value="<%=usuarioId%>" name="usuarioId">
                                <input style="display: none" value="1" name="destinatarioId">
                                <textarea input type="text" name="mensagem" rows="1" cols="120" style="resize: none; border-radius:25px; padding:8px; font-size:15px;" /></textarea><input type="file" name="imagemArquivo" >
        ...
        <div id="optionsPopUp">
            <div align="right"><button onclick="hideOptionsPopUp()" >&times;</button></div>
            <form name="form_acoes_mensagem" method="post">
                <input type="hidden" id="messageIdInput" name="messageId">
                <p><input type="text" size="50" id="passwordInput"style="padding:8px; font-size:15px; border-radius:25px;" name="mensagem" placeholder="Insira sua nova mensagem"></p>
                <button type="submit" class="estilo-botao-verde" formaction="alterar_mensagem.do?idDestinatario=0">Alterar mensagem</button>
                <button type="submit" class="estilo-botao-vermelho" formaction="excluir_mensagem.do?idDestinatario=0">Excluir mensagem</button>
            </form>
        </div>
</div>
        <script>
            ...
            function atualizarConversa() {
                var destinatarioId = 1; // Substitua pelo ID do destinatário desejado
                $.ajax({
...
function exibirMensagensNaInterface(mensagens) {
    ...
    for (var i = 0; i < mensagens.length; i++) {
        ...
        if (mensagem.imagemArquivo !== null && mensagem.imagemArquivo.length > 0) {
